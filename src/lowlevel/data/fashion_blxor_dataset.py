...
class FASHIONBLXORDataset(FASHIONDataset):
	...
	def allign_data_for_func(self,  add_y = False):

		relational_func = lambda a,b: a != b 
	...
	def __str__(self):
		return 'domains: 2, num_points: {}, domain_x {}'.format(self.num_data_points, super(FASHIONBLXORDataset, self).__str__())
	...
	def __getitem__(self, index):
		...
		return {'domain_a': super(FASHIONBLXORDataset, self).__getitem__(self.domain_a[index]),\
				 'domain_b': super(FASHIONBLXORDataset, self).__getitem__(self.domain_b[index]), \
