...
class TRANSFERMNISTDataset(BaseDataset):
	...
	def __init__(self, opt, type_of_data, mydir = None):
		...
		self.A_inputs = loaded['inputs'].astype(np.float32)
		if np.max(self.A_inputs) > 1:
			self.A_inputs = self.inputs / 255.0
		self.A_targets = loaded['targets'].astype(np.float32)

		# print(np.min(self.inputs), np.max(self.inputs))

		self.num_data_points = int(self.A_inputs.shape[0] / self.batch_size) * self.batch_size
	...
	def __getitem__(self, index):
		...
		"""
		inputs_batch = self.A_inputs[index].reshape(*self.get_input_shape())
		# print(index, self.targets[index], type(index), type(self.targets[index]))
		targets_batch = self.to_one_of_k(int(self.A_targets[index]))

		inputs_batch = torch.Tensor(inputs_batch).float()
		targets_batch = torch.Tensor(targets_batch).float()

		return {'A': inputs_batch, 'A_targets': targets_batch, 'A_indexs': index}
	...
	def __str__(self):
		return 'A_inputs_shape: {}, A_target.shape: {}'.format(self.A_inputs.shape, self.A_targets.shape)
