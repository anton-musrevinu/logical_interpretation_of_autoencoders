...
def graph_learning(experiment_name):
	os.system('python {} --experiment_name {} --phase graph'.format(LOWLEVEL_CMD,experiment_name))
...
def do_generative_query(experiment_dir_path, cluster_id, task_type, test = False, type_of_query = 'bin'):

	print('[DOINGIN GENERATIVE QUERY WITH ARGS: {}, {}, {}, {}'.format(experiment_dir_path, cluster_id, task_type, type_of_query))
	
	if task_type == 'classification' or 'noisy' in task_type:
		do_generative_query_for_labels(experiment_dir_path, cluster_id, task_type = task_type, type_of_query = type_of_query)
	elif task_type == 'succ':
		do_generative_query_on_test(experiment_dir_path, cluster_id, task_type = task_type, type_of_query = type_of_query, test = True, fl_to_query = ['fla'])
		do_generative_query_on_test(experiment_dir_path, cluster_id, task_type = task_type, type_of_query = type_of_query, test = True, fl_to_query = ['flb'])
	else:
	...
	do_decode_class_samples(experiment_dir_path, cluster_id, task_type)
...
def do_generative_query_for_labels(experiment_dir_path, cluster_id, test = False, type_of_query = 'bin', task_type = 'classification'):
	...
	for i in range(10):
		...
		y_vec[:,i] += 1
		write_fl_batch_to_file(file_name, np.zeros((100, fl_info[fl_to_query].nb_vars, fl_info[fl_to_query].var_cat_dim)), y_vec, 0)
...
def do_decode_class_samples(experiment_dir_path, cluster_id, task_type):
	psdd_out_dir = _get_psdd_out_dir(experiment_dir_path, cluster_id, task_type)
...
def decode_all_possible():
	...
	for i in toclassify:
		do_decode_class_samples(i[0],i[1], task_type = i[2])
		# break
...
def sample_all_missing():
			...
			for experiment_dir_path, dir_names_2, file_names_2 in os.walk(os.path.join(root, exp)):
				for psdd_search_dir in dir_names_2:
					if 'psdd_search' in psdd_search_dir:
						...
						evaluationDir = os.path.abspath(os.path.join(exp_cluster_dir, './evaluation'))
						if not os.path.exists(evaluationDir):
							toclassify.append((experiment_dir_path, cluster_id, task_type, 'bin'))
							toclassify.append((experiment_dir_path, cluster_id, task_type, 'dis'))
							# print('added because evaldir does not exist', evaluationDir)
						else:
							for root_3, dir_names_3, file_names_3 in os.walk(evaluationDir):
	...
	for i in toclassify:
		if i[2] != 'classification' and i[2] != 'plus':
			do_generative_query(i[0], i[1], i[2], type_of_query = i[3])
	for i in toclassify:
		if i[2] == 'classification':
			do_generative_query(i[0], i[1], i[2], type_of_query = i[3])
...
def do_everything(experiment_dir_path, cluster_id, task_type = 'classification',\
	...
	if task_type == 'classification':
		do_generative_query_for_labels(experiment_dir_path, cluster_id)
	else:
		#Generate class samples and decode them to png
		do_generative_query_on_test(experiment_dir_path, cluster_id, task_type = task_type, type_of_query = type_of_query, test = True, fl_to_query = ['fla'], y_condition = [0])
		do_generative_query_on_test(experiment_dir_path, cluster_id, task_type = task_type, type_of_query = type_of_query, test = True, fl_to_query = ['fla'], y_condition = [1])
